<?php
/**
 * haobao Customizer functionality
 *
 * @package WordPress
 * @subpackage haobao
 * @since haobao 1.0
 ...
 */
function haobao_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'haobao_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'haobao' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => haobao_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'haobao' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'haobao' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'haobao' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'haobao' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'haobao' );
}
add_action( 'customize_register', 'haobao_customize_register', 11 );

/**
 * Register color schemes for haobao.
 *
 * Can be filtered with {@see 'haobao_color_schemes'}.
 *
 ...
 */
function haobao_get_color_schemes() {
	return apply_filters( 'haobao_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'haobao' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'haobao_get_color_scheme' ) ) :
/**
 * Get the current haobao color scheme.
 *
 * @since haobao 1.0
 *
 * @return array An associative array of either the current or default color scheme hex values.
 */
function haobao_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = haobao_get_color_schemes();
...
endif; // haobao_get_color_scheme

if ( ! function_exists( 'haobao_get_color_scheme_choices' ) ) :
/**
 * Returns an array of color scheme choices registered for haobao.
 *
 * @since haobao 1.0
 *
 * @return array Array of color schemes.
 */
function haobao_get_color_scheme_choices() {
	$color_schemes                = haobao_get_color_schemes();
...
endif; // haobao_get_color_scheme_choices

if ( ! function_exists( 'haobao_sanitize_color_scheme' ) ) :
/**
 ...
 */
function haobao_sanitize_color_scheme( $value ) {
	$color_schemes = haobao_get_color_scheme_choices();
...
endif; // haobao_sanitize_color_scheme
 ...
 */
function haobao_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = haobao_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = haobao_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = haobao_get_color_scheme_css( $colors );

	wp_add_inline_style( 'haobao-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'haobao_color_scheme_css' );
 ...
 * @since haobao 1.0
 */
function haobao_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', haobao_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'haobao_customize_control_js' );
 ...
 * @since haobao 1.0
 */
function haobao_customize_preview_js() {
	wp_enqueue_script( 'haobao-customize-preview', get_template_directory_uri() . '/assets/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'haobao_customize_preview_js' );
 ...
 */
function haobao_get_color_scheme_css( $colors ) {
 ...
 * @since haobao 1.0
 */
function haobao_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-haobao-color-scheme">
		<?php echo haobao_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'haobao_color_scheme_css_template' );
